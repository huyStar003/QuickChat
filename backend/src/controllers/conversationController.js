...
export const addMembersToGroup = async (req, res) => {
  try {
    ...
    if (newMemberIds.length === 0) {
      return res.status(400).json({ message: "Tất cả users đã là thành viên" });
    }
    ...
    // Initialize unreadCounts for new members
    ...
    // Populate and format
    await conversation.populate([
      { path: "participants.userId", select: "displayName avatarUrl lastSeen" },
      { path: "lastMessage.senderId", select: "displayName avatarUrl" },
    ]);
    ...
    io.to(conversationId).emit("members-added", {
      conversation: formatted,
      addedBy: userId,
      newMembers: newMemberIds,
    });

    // 2. Notify new members and make them join room
    newMemberIds.forEach((memberId) => {
      io.to(memberId.toString()).emit("added-to-group", formatted);
    });
